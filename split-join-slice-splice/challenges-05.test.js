...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
      let a = recipe.ingredients[i].indexOf(" ");
      temp.push(recipe.ingredients[i].slice(a+1));
  }
  for (let j = 0; j < temp.length; j++) {
      let b = temp[j].indexOf(" ");
      result.push(temp[j].slice(b+1));
  }
  return result;
}
...
const splitFoods = (recipe) => {
  let result = [];
  for (let i = 0; i < recipe.ingredients.length; i++) {
      let a = recipe.ingredients[i].split(" ", 2).join(" ").length;
      result.push(recipe.ingredients[i].slice(a+1));
  }
  return result;
}
...
const stepActions = (recipe) => {
  ...
  for (let i = 0; i < recipe.steps.length; i++) {
    let a = recipe.steps[i].indexOf(" ");
    result.push(recipe.steps[i].slice(0,a));
}
  return result;
}
