...
const count = (target, input) => {
  return input.reduce((acc,val) => acc.concat(val), []).filter(x => x === target).length;
};
...
const totalSum = (input) => {
  return input.reduce((acc,val) => acc.concat(val)).reduce((total,sum) => total + sum);
};
...
const divisibleByFiveTwoToThePower = (input) => {
    return input.filter(nest => nest.filter(x => {
        if (typeof x == 'number' && x % 5 === 0) {
            return x;
        }
    })).map(nested => nested.map(e => e ** 2));
...
let findMaleAndFemale = (data) => {
  let people = [];
  data.reduce((x,y) => {
      if (y.gender == 'male' || y.gender == 'female') {
          people.push(y.name);
      }
      return x;
  }, {});
  return people.slice().join(' and ');
}
