...
const filterStringsWithVowels = (arr) => {
  return arr.filter(x => {if (x.match(/[aeiou]/gi)) {
      return x;
    }
  });
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(x => {
      if (forbiddenValues.indexOf(x) == -1) {
          return x;
      }
  });
};
...
const getStatName = (arr, minBaseStat) => {
    var newArr = [];
    arr.filter(x => {if (x.baseStat > minBaseStat)
        newArr.push(x.stat.name);
    });
    return newArr;
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(x => {if (!x.children) {
      return x.name;
  }});
};
